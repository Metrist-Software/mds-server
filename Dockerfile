...
RUN echo Revision: ${TAG} >priv/static/build.txt
RUN echo Build-Date: `date` >>priv/static/build.txt
...
RUN apt-get update -y && apt-get install -y libstdc++6 openssl libncurses5 locales wget unzip openssh-client \
  && apt-get clean && rm -f /var/lib/apt/lists/*_*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/mds_server ./
